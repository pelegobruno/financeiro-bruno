...
function abrirModal(titulo, texto, corBotao, acaoConfirmar, precisaInput = true) {
    ...
    btnConfirmar.disabled = false; // Garante que o botão abre ativo
    btnConfirmar.innerHTML = '<i class="fas fa-check"></i> Confirmar'; // Restaura o texto padrão ao abrir
    
    btnConfirmar.onclick = async () => {
        // Executa a ação contida no botão se ele não estiver travado
...
async function verDetalhes(id, nome) {
    try {
        ...
        // LÓGICA DE VENCIMENTO DINÂMICO (PARCELAMENTO)
...
async function salvarRegistro() {
    ...
    if (btn.disabled) return; // BLOQUEIO CENTRAL DE SEGUNDO CLIQUE RAPIDO
    ...
    try {
        btn.disabled = true; // Desativa o botão imediatamente
        ...
        btn.disabled = false; // Só libera o botão após o término completo
...
async function adicionarAcrescimo(id, nome) {
    abrirModal("Lançar Taxa Extraordinária", `Defina o montante avulso para ${nome}:`, "var(--warning)", async () => {
        ...
        const btnConfirmar = document.getElementById('modal-btn-confirmar');
        if (btnConfirmar.disabled) return; // BLOQUEIO CENTRAL DE SEGUNDO CLIQUE RAPIDO DO MODAL
        
        try {
            btnConfirmar.disabled = true; // Desativa instantaneamente
...
function gerarRelatorioCliente(clienteNome) {
    ...
    const analiseHTML = `
        ...
        <div style="background: rgba(255,255,255,0.05); border: 1px solid ${corDecisao}; border-radius: 12px; padding: 15px; margin: 15px 0;">
            ...
            <p style="font-size: 12px; color: #cbd5e1; margin-bottom: 10px; line-height: 1.4;">Com base na capacidade de pagamento comprovada e apetite de risco da operation, o Cérebro Central calculou a margem segura para novas emissões.</p>
